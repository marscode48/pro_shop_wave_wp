...
本テーマおよび EC サイトは、以下のプラグイン群を前提として設計・運用されています。  
新規環境構築・引き継ぎ時は、**最低限「必須」プラグインの導入を行ってください。**

### 必須（EC 運用の前提）

- WooCommerce  
  商品管理・カート・決済など、EC 機能の中核

- WooCommerce Stripe ゲートウェイ  
  クレジットカード決済（Stripe）対応
...
### 多言語対応（多言語運用を行う場合のみ必須）

- WPML（WordPress Multilingual Plugin）  
  サイト全体の多言語対応を行うための中核プラグイン。  
  商品名・商品説明・カテゴリ・タグ・固定ページなどの翻訳管理に使用。

- WooCommerce Multilingual & Multicurrency（WPML アドオン）  
  WooCommerce 商品・属性・バリエーション・スラッグを多言語で管理するために必須。

※ 日本語単一運用の場合は必須ではありませんが、  
※ 多言語対応を行う場合は **後から追加せず、初期段階で導入することを強く推奨** します。  
※ WPML 導入後は、スラッグ命名・画像命名・属性設計のルール変更が発生するため、  
　本 README の命名規則・運用ルールを必ず参照してください。
